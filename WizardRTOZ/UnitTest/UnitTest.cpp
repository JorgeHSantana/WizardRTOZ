...
void UnitTest::run(bool abort_at_error, std::function<void(char)> write_function){
    ...
    if (error_totalizer == 0){
        UnitTest::log("[v] All tests passed!");
    } else {
        UnitTest::log("[x] All tests were run and %d error%c %s found.", error_totalizer, error_totalizer > 1 ? 's' : 0, error_totalizer > 1 ? "were" : "was");
    }
}
